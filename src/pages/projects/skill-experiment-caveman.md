...
결론부터 말하면 나는 이 스킬을 <mark>조건부로 쓴다.</mark> 저자가 예고한 대로 방향이 정확히 갈렸다 — 출력이 긴 작업에서는 세션 비용이 15.1% 줄었고, 출력이 짧은 작업에서는 9.9% 늘었다. 아래는 그 갈림을 어떻게 확인했는지, 그리고 그게 내 실제 작업에서는 무엇을 뜻하는지에 대한 기록이다.
